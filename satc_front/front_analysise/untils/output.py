...
class RTOSOutput():
    ...
    def __init__(self, result, out_dir):
        self.baseoutdir = out_dir
        self.detailoutput = self.baseoutdir
        self.simpleoutput = self.baseoutdir
        self.result = result

    def write_high_precision(self):
        self.keywords = set()
        self.function = set()
        self.other = set()
        self.other1 = set()
        with open(os.path.join(self.detailoutput, "highprecision.result"), "w") as f:
            for res in self.result:
                keywords = set()
                function = set()
                for r in res["keywords"]:
    ...
    def write_low_precision(self):
        with open(os.path.join(self.simpleoutput, "lowprecision.result"), "w") as f:
            for args in args_set:
                f.write("{}\n".format(args.name))