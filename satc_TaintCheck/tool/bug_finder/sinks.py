...
def doSystemCmd(p,core_taint,plt_path,*_,**__):
    ...
    if are_parameters_in_registers(p):
        ...
        for i in range(1, 1 + n_vargs):
            ...
            print name,':',reg
            if (core_taint.is_tainted(reg, path=plt_path) or checkstringtainted(p,core_taint,plt_state,name,plt_path)):
...
def strcpy(p, core_taint, plt_path, size_con=None):
    ...
    if are_parameters_in_registers(p):
        ...
        if core_taint.is_tainted(reg_src, plt_path) or checkstringtainted(p,core_taint,plt_state,name_reg_src,plt_path):
