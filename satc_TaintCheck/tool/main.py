...
def main_main():
    ...
    '''
    with open("./r700p_http.pk",'wb') as f:
        pickle.dump((proj,cfg),f)
    with open("./ac18",'rb') as f:
        ...
        try:
            ...
            if proj.arch.name!="MIPS32":        
                if not proj.loader.main_object.pic:
                    ...
                    start_addr=cfg.get_any_node(int(cont[i*3].split(' ')[1],0)-0x10000+0x400000,anyaddr=True).addr
                    callerbb=None
            else:
                if not proj.loader.main_object.pic or "system.so" in proj.filename:
                    ...
                    taint_addr=taint_addr-0x10000+0x400000
                    sinkTargets=[j-0x10000+0x400000 for j in sinkTargets]
                    start_addr=cfg.get_any_node(int(cont[i*3].split(' ')[1],0)-0x10000+0x400000,anyaddr=True).function_address
                    callerbb=cfg.get_any_node(int(cont[i*3].split(' ')[1],0)-0x10000+0x400000,anyaddr=True).addr
                    with open(binary,'rb') as f:
                        conttmp=f.read()
                        sec=proj.loader.main_object.sections_map['.got']
                        proj.loader.memory.write_bytes(sec.min_addr,conttmp[sec.offset: sec.offset+sec.memsize])
            ...
            if getfindflag()[0]:
                ...
                res="0x%x 0x%x "%(taint_addr,start_addr)+"  found : %s"%" ".join([hex(i) for i in set(getfindflag()[1])])
            else:
